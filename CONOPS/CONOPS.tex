...
\lfoot{\vspace{-1.0cm} February 10, 2016}
...
\begin{tabular}{|c|c|c|}
    ...
    Luke Alexander  &  Chief Systems Engineer     &  LukeAlexander@gatech.edu    \\ \hline
    Sandeep Vijayasekar    &  FSW Lead  &  sand500@gatech.edu  \\ \hline
    Lubna Zubair & CDH Lead & lubnaz@gatech.edu \\ \hline
    Alex Akins & COM Lead & aakins6@gatech.edu \\ \hline
\end{tabular}
\end{center}
...
\underline{Overview:} The Eclipse Data Processing mode is very similar to Sunlight Data Processing in that it processes whatever is left of the payload image data after it has been gathered and saved to disk. However, there is a difference in hardware used during this mode as the spacecraft should be in a lower power state while in eclipse so as not reach an unacceptable depth of discharge. It is assumed that the spacecraft will not be collecting data while it is in eclipse, so if there is no data to be processed, this mode will not be entered.
...
\underline{Overview:} Upon completion of Eclipse Transmit mode (or having never entered it) the spacecraft will then complete the next pointing command relevant to the gathering of payload data. Given that the spacecraft is in eclipse, this will be towards one of the poles as opposed to towards the equator. Upon completing this slew maneuver, the spacecraft will wait until entering sunlight and further payload operations.
